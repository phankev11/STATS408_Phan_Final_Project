...
-   There appears to be a slight linear relationship between `subtotal` and `delivery_time` (*See Appendix 2.IV.B.*)
...
-   There appears to be a slight difference in `delivery_time` between the different `market_id`s (*See Appendix 2.IV.F.*)

-   There appears to be a slight difference in `delivery_time` between the `meal_time`s (*See Appendix 2.IV.G.*)
...
### 2.IV.B. Scatter plot of delivery time by subtotal

```{r}
# Scatter plot of delivery time by subtotal
delivery_clean %>%
  ggplot(aes(x = subtotal, y = delivery_time)) +
  geom_point() +
