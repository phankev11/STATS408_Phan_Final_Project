...
We find that removing the interaction term between `total_items` and `meal_time` maximizes the adjusted-$R^2$:

```{r}
# Create best model
mod_best <- lm(log(delivery_time) ~ total_items + I(total_items^2) + 
                                    max_item_price + total_busy_dashers +
                                    total_outstanding_orders +
                                    estimated_store_to_consumer_driving_duration +
                                    market_id + meal_time + I(total_items^2) *
                                    meal_time + max_item_price * meal_time +
                                    total_busy_dashers * meal_time +
                                    total_outstanding_orders * meal_time +
                                    estimated_store_to_consumer_driving_duration *
                                    meal_time + market_id * meal_time,
...
var_sel <- regsubsets(log(delivery_time) ~ total_items + I(total_items^2) + 
                                   max_item_price + total_busy_dashers +
                                   total_outstanding_orders +
                                   estimated_store_to_consumer_driving_duration +
                                   market_id + meal_time + total_items *
